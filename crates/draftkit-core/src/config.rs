...
//! - `.draftkit.<ext>` in current directory or any parent
//! - `draftkit.<ext>` in current directory or any parent
//! - `~/.config/draftkit/config.<ext>` (user config)
//!
//! Where `<ext>` is one of: `toml`, `yaml`, `yml`, `json`, `json5`, `ini`
//!
//! # Example
//! ```no_run
...
/// The configuration for draftkit.
///
/// Add your configuration fields here. This struct is deserialized from
/// config files found during discovery (TOML, YAML, JSON, JSON5, or INI).
...
pub struct Config {
    ...
    /// OpenTelemetry OTLP endpoint; when set, enables trace export.
...
const CONFIG_EXTENSIONS: &[&str] = &["toml", "yaml", "yml", "json", "json5", "ini"];
...
impl ConfigLoader {
    ...
    /// Set whether to include user config from `~/.config/draftkit/`.
    ...
    /// 3. User config (`~/.config/draftkit/config.<ext>`)
    /// 4. Default values
    ...
    fn find_project_config(&self, start: &Utf8Path) -> Option<Utf8PathBuf> {
        ...
        while let Some(dir) = current {
            ...
            for ext in CONFIG_EXTENSIONS {
                // Try dotfile first (.draftkit.toml)
                let dotfile = dir.join(format!(".{APP_NAME}.{ext}"));
                if dotfile.is_file() {
                    return Some(dotfile);
                }

                // Then try regular name (draftkit.toml)
    ...
    fn find_user_config(&self) -> Option<Utf8PathBuf> {
        ...
        for ext in CONFIG_EXTENSIONS {
            let config_path = app_config_dir.join(format!("config.{ext}"));
...
///
/// Returns `~/.config/draftkit/` on Unix, equivalent on other platforms.
pub fn user_config_dir() -> Option<Utf8PathBuf> {
    let config_dir = dirs::config_dir()?;
    Utf8PathBuf::from_path_buf(config_dir.join(APP_NAME)).ok()
}
...
"#,
    ...
    fn test_project_config_discovery() {
        ...
        let config_path = project_dir.join(".draftkit.toml");
    ...
    fn test_boundary_marker_stops_search() {
        ...
        fs::write(parent.join(".draftkit.toml"), r#"log_level = "warn""#).unwrap();
    ...
    fn test_explicit_file_overrides_project_config() {
        ...
        // Project config
        let project_config = tmp.path().join(".draftkit.toml");
